...
function(declare, lang, array, html, query, on, json, Memory, Deferred, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin,
  Tooltip, template, TextBox, esriRequest, esriLang, locale, PagingQueryTask, FilteringSelect, ValidationTextBox, DateTextBox,
  NumberTextBox) {/*jshint unused: false*/
  return declare([_WidgetBase,_TemplatedMixin,_WidgetsInTemplateMixin], {
    baseClass: 'jimu-widget-singlevalue-setting',
    ...
    _onFieldsSelectChange:function(value){
    ...
    _bindEvents:function(){
      ...
      this.own(on(this.btnCancel,'click',lang.hitch(this,function(){
        if(this._def){
          this._def.cancel('cancel button was clicked');
        }
        if(this._isAddNow){
          this.onAddCancel();
        }
    ...
    _resetUniqueValuesSelect:function(){
      ...
      if(this.config && fld && fld.name === this.config.fieldObj.name && this.xThru > 1){
        return;
      }
      ...
      if(this._layerInfo){
        var version = parseFloat(this._layerInfo.currentVersion);
/* generateRenderer does not return a unique list of attributes all the time so this block of code has been commented out to
   use my PagingQueryTask instead */
        /*if(version >= 10.1){
          var item = (this.fieldsSelect) ? this._getSelectedFilteringItem(this.fieldsSelect) : null;
          if(item){
            var url = this.layerURL + "/generateRenderer";
            ...
            esriRequest({
              ...
              if(uniqueValueInfos && item.id === fieldInfo.id){
                ...
                if(!this.isValueRequired){
                  var dataItem3 = lang.mixin({}, {id:'',name:''});
                  dataItem3.id = data.length - 1;
                  data.push(dataItem3);
                }
