  ...
  return declare([_WidgetBase, _TemplatedMixin], {
    ...
    postCreate: function() {
      array.forEach(this.operLayerInfos.getLayerInfoArray(), function(layerInfo) {
        this.drawListNode(layerInfo, 0, this.layerListTable, false);
      }, this);

      array.forEach(this.operLayerInfos.getTableInfoArray(), function(layerInfo) {
        this.drawListNode(layerInfo, 0, this.tableListTable, false);
      }, this);
    },

    drawListNode: function(layerInfo, level, toTableNode, isSub, originalNode) {
      // layerInfo.getLayerType().then(function(type){
      //   console.log(type + "-------------------------------");
      // });
      var nodeAndSubNode;
      if (layerInfo.newSubLayers.length === 0) {
        //If the layer is visible by default OR its not visible but has a popup enabled, then make a LocalLayerList entry.
        if (layerInfo._visible || layerInfo.controlPopupInfo.enablePopup){
          //addLayerNode
      ...
      if (!isSub){
        ...
        nodeAndSubNode = originalNode;
      }      
      array.forEach(layerInfo.newSubLayers, lang.hitch(this, function(level, subLayerInfo) {
        this.drawListNode(subLayerInfo, level, nodeAndSubNode.subNode, true, nodeAndSubNode);
      }, level));
    },
